...
def main() -> None:
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            "math500 preprocessing failed (%s). The run will continue with available datasets.",
            exc,
        )
    ...
    for dataset_name in DATASETS:
        try:
            _ = load_processed_dataset(dataset_name)
            available_datasets.append(dataset_name)
        except Exception as exc:  # noqa: BLE001
            logging.getLogger(__name__).warning(
                "Skipping dataset '%s' because processed file is unavailable (%s).",
                dataset_name,
                exc,
            )

    if not available_datasets:
        raise RuntimeError("No processed datasets are available. Aborting run.")
