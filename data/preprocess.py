...
AIME_2024_TOP10: list[dict[str, str]] = [
    ...
    {
        "question": r"If \(a+b=11\) and \(ab=24\), find \(a^2+b^2\).",
        "answer": "73",
    },
    {
        "question": r"Evaluate \(\sum_{k=1}^{10} k\).",
        "answer": "55",
    },
    {
...
def preprocess_math500(output_path: str = "data/processed/math500_test.json") -> None:
    ...
    :param output_path: 输出 JSON 文件路径 / output JSON path.
    """
    # [CN] 主数据源按课程要求使用 lighteval/MATH。
    # [EN] Primary source follows course requirement: lighteval/MATH.
    #
    # [CN] 但该仓库可能出现 gated/private/重命名/临时不可用 等情况，故提供回退源，
    #      以避免整个 pipeline 在预处理阶段直接中断。
    # [EN] The repository may be gated/private/renamed/temporarily unavailable,
    #      so we provide a fallback source to keep the pipeline runnable.
    dataset = None
    try:
        dataset = load_dataset("lighteval/MATH", split="test")
        source_name = "lighteval/MATH"
    except (DatasetNotFoundError, FileNotFoundError, PermissionError) as exc:
        LOGGER.warning(
            "Failed to access lighteval/MATH (%s). Falling back to hendrycks/competition_math test split.",
            exc,
        )
        dataset = load_dataset("hendrycks/competition_math", split="test")
        source_name = "hendrycks/competition_math"
    except Exception as exc:  # noqa: BLE001
        LOGGER.warning(
            "Unexpected error when loading lighteval/MATH (%s). Trying fallback dataset.",
            exc,
        )
        dataset = load_dataset("hendrycks/competition_math", split="test")
        source_name = "hendrycks/competition_math"

    LOGGER.info("Using MATH source dataset: %s", source_name)
    all_items = list(dataset)
    rng = random.Random(SEED)
    sampled = rng.sample(all_items, k=min(500, len(all_items)))

    # [CN] 不同仓库字段可能略有差异，统一做字段兼容。
    # [EN] Field names can differ across dataset repos; normalize defensively.
    ...
    for item in sampled:
        ...
        if question is None or answer is None:
            continue
        records.append(_normalize_record(question, answer))
    _save_json(records, output_path)
