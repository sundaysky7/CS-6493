...
def preprocess_math500(output_path: str = "data/processed/math500_test.json") -> None:
    """
    中文（版本1）：
        下载 HuggingFaceH4/MATH-500 数据集并写出标准化结果。
        该数据集本身即为 500 条样本，因此无需再次随机抽样。

    English (Version 2):
        Download the HuggingFaceH4/MATH-500 dataset and save normalized results.
        The dataset already contains 500 samples, so no additional sampling is needed.

    :param output_path: 输出 JSON 文件路径 / output JSON path.
    """
    dataset = load_dataset("HuggingFaceH4/MATH-500", split="test")

    records = [_normalize_record(item["problem"], item["solution"]) for item in dataset]
    _save_json(records, output_path)
